...
- **WHEN**: `nvidia-smi` or the GPU/driver is gone, or `ssh bard@192.168.x.x` no longer works.
...
- Host `pop-os`, user `bard`, reached from the Mac via `ssh bard@192.168.x.x` — or the config
  alias `ssh pop-os`, which is current and works (run through the `system` MCP `system_exec`).
  ⚠️ STALE-CORRECTED 2026-08-23: this file previously wrote the command as `user@192.168.x.x`
  throughout, which FAILS with "Permission denied (publickey)". The user is `bard`. It also
  contradicted itself — this very line already said user `bard` while the command said `user`.
  Canonical: [[pop-access-canonical-2026-08-23]].
...
7. **Confirm remote access**: ensure `sshd` is running (`sudo systemctl enable --now ssh`) and `ssh bard@192.168.x.x` works from the Mac (this is what `system_exec` drives).
...
- ✅ `ssh bard@192.168.x.x` works from the Mac (system_exec path restored).
